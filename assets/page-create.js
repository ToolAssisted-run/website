...
if (createGameForm) {
  ...
  wireCreateForm(
    createGameForm,
    document.getElementById('cg-login'),
    createGameMsg,
    '/api/game/create',
    function (j) {
...
if (createCatForm) {
  var catGameTitles = JSON.parse(
    document.getElementById('ccgamedata').textContent
  );
  var catGameKey = null;
  try {
    catGameKey = new URLSearchParams(location.search).get('game');
  } catch (e) {}
  if (!catGameKey || !catGameTitles[catGameKey]) {
    ...
    if (ccGamenameEl || ccCrumbGameEl) {
      ...
      if (ccGamenameEl) {
        ...
        try {
          if (
            ccGamenameEl.tagName &&
            ccGamenameEl.tagName.toLowerCase() === 'a'
          ) {
      ...
      if (ccCrumbGameEl) {
        ...
        try {
          if (
            ccCrumbGameEl.tagName &&
            ccCrumbGameEl.tagName.toLowerCase() === 'a'
          ) {
    ...
    function paintParent() {
      ...
      ruleIn.placeholder = isSub
        ? 'What this subcategory adds to the category rule (optional)'
        : 'What must a run do to belong here?';
      createCatForm.querySelector('[name=label]').placeholder = isSub
        ? 'e.g. any%'
        : 'e.g. 100k points';
    }
    ...
    if (ccPvBtn && ccPvBox && ccPvContent) {
      ccPvBtn.addEventListener('click', function () {
        if (ccPvBox.hidden) {
          ...
          post('/api/preview', fd, ccPvBtn)
            .then(function (res) {
              ccPvContent.innerHTML =
                res.ok && res.j.ok
                  ? res.j.html
                  : (res.j && res.j.error) || 'preview failed';
            })
            .catch(function () {
              ccPvContent.textContent =
    ...
    wireCreateForm(
      createCatForm,
      document.getElementById('cc-login'),
      createCatMsg,
      '/api/category/add',
      function (j) {
        ...
        waitBuilt(j.serial, function (live) {
          note(
            createCatMsg,
            live
              ? 'Created. The category is live; submit the run now.'
              : 'Created. It will appear on the site shortly.',
            true
          );
          var a = document.createElement('a');
          a.className = 'btn';
          a.href = '../submit/?game=' + catGameKey;
          ...
          setTimeout(function () {
            ...
            try {
              var lab = createCatForm.querySelector('[name=label]');
