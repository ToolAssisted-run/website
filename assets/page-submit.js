...
function authorNames() {
  authorNamesPromise =
    authorNamesPromise ||
    fetch(rel + 'assets/authornames.json' + versionQuery)
      .then(function (r) {
        return r.json();
      })
      .catch(function () {
...
function initAuthorPick(pick, initial) {
  ...
  var KNOWN = [];
  authorNames().then(function (n) {
    KNOWN = n;
  });
  // the co-author policy link beside the picker appears with the second author
  var coauthNote =
    pick.parentNode && pick.parentNode.querySelector('.coauthnote');
  function sync() {
  ...
  pick.setAuthors = function (names) {
  ...
  fetch(api + '/api/run/record?run=' + encodeURIComponent(moveRunId), {
    ...
    .then(function (record) {
      ...
      function destinationReady() {
        return (
          !!gameField.value &&
          gameField.value !== record.game.key &&
          !!goal.value &&
          (subWrap.hidden || !!sub.value)
        );
      }
      function paint() {
        ...
        agreePanel.classList.toggle(
          'done',
          ready && why.value.trim().length >= 8
        );
      ...
      function loadGoals() {
        createCategory.href =
          '../create-category/?game=' + encodeURIComponent(gameField.value);
        ...
        fetch(api + '/api/categories?game=' + encodeURIComponent(key))
          .then(function (response) {
            ...
            return fetch(
              gameData.raw + '/games/' + key + '/categories.json'
            ).then(function (raw) {
          ...
          .catch(function () {
            note(msg, 'Could not load that game\u2019s categories.', false);
          });
      }
...
if (submitForm) {
  ...
  mePromise.then(function (d) {
    ...
    if (d.unreachable) {
      note(
        msg,
        'The archivist is not reachable right now; try again later.',
        false
      );
      return;
    }
    ...
    if (moveRunId) {
      enterMoveMode(gameData, submitForm);
      return;
    }
    ...
    function composeTime() {
      ...
      var v = timeSegments.map(function (seg) {
        ...
        var max = parseInt(
          seg.getAttribute('max') || seg.dataset.max || '999',
          10
        );
        n = Math.max(0, Math.min(max, n));
        if (seg.id === 't-ms') {
          if (n > max || n < 0)
            seg.value = String(Math.max(0, Math.min(max, n))).padStart(3, '0');
        } else {
    ...
    function markTimeManual() {
      ...
      if (sel) {
        var hasAny = timeSegments.some(function (s) {
    ...
    if (msOptLead) {
      msOptLead.addEventListener('mousedown', function (e) {
        e.preventDefault();
      });
    ...
    if (msOptTrail) {
      msOptTrail.addEventListener('mousedown', function (e) {
        e.preventDefault();
      });
    ...
    var gameSelect = document.getElementById('s-game'),
      goalSelect = document.getElementById('s-goal');
    ...
    function statedDefs() {
      ...
      return (curMetrics || []).filter(function (m) {
        return m.key !== 'time';
      });
    }
    function secsOf(v) {
    ...
    function buildMetricFields() {
      ...
      statedDefs().forEach(function (m) {
        ...
        hiddenField.name = 'metric_' + m.key;
        if (m.type === 'time') {
          ...
          var segs = [
            ...
            inp.min = 0;
            inp.max = segSpec[1];
            inp.placeholder = segSpec[0] === 'h' ? '0' : '00';
            var segLabel = document.createElement('label');
            segLabel.textContent = segSpec[0];
            span.appendChild(inp);
            span.appendChild(segLabel);
            wrap.appendChild(span);
            return inp;
          });
          function compose() {
            var v = segs.map(function (seg) {
              return Math.max(0, parseInt(seg.value, 10) || 0);
            });
            hiddenField.value = String(
              v[0] * 3600 + v[1] * 60 + v[2] + v[3] / 1000
            );
          }
          segs.forEach(function (seg) {
            seg.addEventListener('input', compose);
          });
          ...
          numberInput.addEventListener('input', function () {
            hiddenField.value = String(secsOf(numberInput.value));
          });
          hiddenField.fill = function (v) {
            numberInput.value = v;
            hiddenField.value = String(secsOf(numberInput.value));
          };
          ...
          fromBtn.type = 'button';
          fromBtn.className = 'btn quiet mfrom';
    ...
    movieInput.addEventListener('change', function () {
      ...
      movieInfo = null;
      if (!file) {
        movieNote.hidden = true;
        setMovieMark('', '');
        paintKind();
        paintPanels();
        return;
      }
      movieNote.hidden = false;
      ...
      fetch(api + '/api/movie/inspect', {
          ...
          if (!j.ok) {
            ...
            if (j.parsed) {
              ...
              var why = j.known
                ? '.' +
                  j.format +
                  ' is a known format the archive could not read' +
                  (j.error ? ' (' + j.error + ')' : '')
              ...
              movieNote.textContent =
                '! ' +
                why +
                '. You can continue: the file is archived exactly as it is, ' +
        ...
        .catch(function () {
          movieInfo = {
            error: 'could not reach the archivist to read the movie',
          };
          movieNote.textContent = '✗ ' + movieInfo.error;
          movieNote.className = 'rules fullw enc-bad';
          setMovieMark('bad', movieInfo.error);
    ...
    function renderAttachments() {
      ...
      existingAttachments.forEach(function (att) {
        ...
        if (repFile) {
          ...
          repBadge.textContent =
            repFile.name === att.name
      ...
      if (editRunId && editMay && !editMay.author) {
        ...
        if (attHint)
          attHint.textContent =
            'Supplementary files are editable by authors only.';
      }
    }
    ...
    function stageFiles(fileList) {
      ...
      var errs = [];
      var activeCount =
        existingAttachments.filter(function (a) {
          return !deletedAttachments.has(a.name);
        }).length + stagedAttachments.length;
      var movieCount =
        existingAttachments.filter(function (a) {
        ...
        stagedAttachments.filter(function (f) {
          return ATTACH_MOVIE_EXTS.has(extOf(f.name));
        }).length;
      ...
      Array.prototype.forEach.call(fileList, function (file) {
        ...
        replacedAttachments.forEach(function (rf) {
          if (rf.name === file.name) inReplaced = true;
        });
        var duplicate =
          stagedAttachments.some(function (f) {
            return f.name === file.name;
          }) ||
          inReplaced ||
          (!deletedAttachments.has(file.name) &&
            existingAttachments.some(function (a) {
    ...
    if (attReplaceInput) {
      attReplaceInput.addEventListener('change', function () {
        ...
        var alreadyStaged = stagedAttachments.some(function (f) {
          return f.name === file.name;
        });
        var alreadyExisting = existingAttachments.some(function (a) {
          return (
            a.name === file.name &&
            !deletedAttachments.has(a.name) &&
            a.name !== replaceTargetName
          );
        });
        var alreadyReplaced = false;
        replacedAttachments.forEach(function (rf, target) {
          if (target !== replaceTargetName && rf.name === file.name)
    ...
    function loadAuthoritativeEmulators() {
          ...
          for (var skey in sysMap) {
            ...
            (sdata.tools || []).forEach(function (t) {
              ...
              if (t.versions && t.versions.length)
                p.system_versions[skey] = t.versions;
      ...
      if (elData && elData.textContent.trim()) {
        try {
          ingest(JSON.parse(elData.textContent));
        } catch (e) {}
      }
    ...
    var emuInput =
      document.getElementById('s-emulator') ||
      submitForm.querySelector('[name=emulator]');
    ...
    function findPresetMatch(str) {
      ...
      for (var i = 0; i < EMULATOR_PRESETS.length; i++) {
        ...
        if (s === p.name.toLowerCase() || s.indexOf(p.name.toLowerCase()) === 0)
          return p;
    ...
    function extractVersionFromInput(preset, str) {
      ...
      for (var i = 0; i < names.length; i++) {
        var nm = names[i];
        var verRegex = new RegExp(
          '^(' + escapeRegex(nm) + '[\\s\\-_]+)([^\\s\\(\\+]+)',
          'i'
        );
      ...
      if (!cur) {
        ...
        if (matched && matched.id === preset.id) {
          ...
          if (m) {
            ...
            var nameOnlyRegex = new RegExp(
              '^' + escapeRegex(matched.name) + '([\\s\\-_]*)(.*)$',
              'i'
            );
    ...
    function applyCoreToInput(preset, core) {
      ...
      if (m) {
        ...
        if (existingCore.toLowerCase() === core.toLowerCase()) {
          ...
          emuInput.value = cur
            .replace(corePattern, ' (Core: ' + core + ')')
            .trim();
        }
      } else {
        if (!cur) {
          emuInput.value =
            (preset ? preset.name : '') + ' (Core: ' + core + ')';
    ...
    function updateActiveChips(preset) {
      ...
      emuVersionsBox
        .querySelectorAll(
          '.emu-chip:not(.emu-chip-core):not(.emu-chip-add-core):not(.emu-chip-add-ver)'
        )
    ...
    function renderVersionsAndCores(preset) {
      ...
      if (
        !preset &&
        !curVal &&
        (!emuPresetSel || emuPresetSel.value !== '__custom__')
      ) {
      ...
      if (activeCore) {
        var exists = cores.some(function (c) {
          return c.toLowerCase() === activeCore.toLowerCase();
        });
        if (!exists) {
      ...
      var sysVers =
        (preset &&
          preset.system_versions &&
          curSys &&
          preset.system_versions[curSys]) ||
      ...
      if (sysVers && sysVers.length) {
        sysVers.forEach(function (ver) {
          ...
          chip.addEventListener('click', function (ev) {
            ev.preventDefault();
            var curActive = extractVersionFromInput(
              preset,
              emuInput ? emuInput.value : ''
            );
      ...
      var supportsCores =
        preset &&
        (preset.has_cores ||
          (preset.cores && Object.keys(preset.cores).length > 0));
      if (supportsCores) {
    ...
    function updateQuickChipsActive(val) {
      ...
      emuQuickBox.querySelectorAll('.emu-chip').forEach(function (btn) {
        var txt = (btn.dataset.val || btn.textContent || '')
          .trim()
          .toLowerCase();
    ...
    function updateEmulatorAssistant(systemKey) {
      ...
      var sysName =
        (systemNames && systemNames[sys]) || (sys ? sys.toUpperCase() : '');
      ...
      if (otherPresets.length) {
        ...
        otherPresets.forEach(function (p) {
          if (recPresets.indexOf(p) >= 0 || multiPresets.indexOf(p) >= 0)
            return;
          var opt = document.createElement('option');
          opt.value = p.id;
          opt.textContent = p.name;
      ...
      if (gameToolPresets.length) {
        ...
        var gameCount = 0;
        gameToolPresets.sort(function (a, b) {
      ...
      if (emuQuickBox) {
        ...
        var chips =
          sys && QUICK_CHIPS[sys] ? QUICK_CHIPS[sys] : QUICK_CHIPS['default'];
      ...
      if (emuManageLink) {
        mePromise.then(function (d) {
          ...
          if (isCurator) {
            emuManageLink.href = sys
              ? rel + 'systems/' + sys + '/#sys-emu-curate'
              : rel + 'tools/';
          }
        });
      }
    ...
    function initEmulatorAssistant() {
      ...
      emuPresetSel.addEventListener('change', function () {
        ...
        var preset = null;
        for (var i = 0; i < EMULATOR_PRESETS.length; i++) {
          if (EMULATOR_PRESETS[i].id === val) {
            preset = EMULATOR_PRESETS[i];
            break;
          }
        }
        if (!preset) return;
        ...
        if (!firstVer) {
          var verInp = emuVersionsBox
            ? emuVersionsBox.querySelector('.emu-version-input')
            : null;
          if (verInp) {
            verInp.focus();
    ...
    function importSources() {
      return {
        movie: (movieInfo && movieInfo.parsed && movieInfo.seconds) || null,
    ...
    function paintTimeImport() {
      if (timeImportSel) {
        ...
        timeImportSel.title = any
          ? 'Fill the time from a source the form has checked'
    ...
    if (timeImportSel)
      timeImportSel.addEventListener('change', function () {
    ...
    function paintKind() {
      ...
      movieWrap.hidden = !!(
        editRunId &&
        (hasMovie ? !mayReplace : !(mayReplace || (editMay && editMay.author)))
      );
      ...
      scoreNote.textContent =
        goalSelect.value === 'unclassified'
          ? 'Unclassified runs rank by likes alone: nothing to score.'
          : stated
            ? n
              ? 'This category ranks by time, which you state, and by the values below.'
              : 'This category ranks by time, which you state.'
    ...
    function fillGoals(goals, loaded) {
      ...
      u.value = 'unclassified';
      u.textContent = 'Unclassified (no goal; ranked by likes)';
    ...
    function loadGoals() {
      ...
      if (createCategoryBtn) {
        if (gameSelect.value) {
          createCategoryBtn.href =
            '../create-category/?game=' + encodeURIComponent(gameSelect.value);
      ...
      if (goalCache[gameSelect.value]) {
        fillGoals(goalCache[gameSelect.value], true);
        return;
      }
    ...
    var subWrap = document.getElementById('s-subwrap'),
      subSelect = document.getElementById('s-sub');
      ...
      if (editRecord)
        setTimeout(function () {
          ...
          Object.keys(mv).forEach(function (k) {
            ...
            if (!h) return;
            if (h.fill) h.fill(mv[k]);
            else h.value = mv[k];
          });
          paintPanels();
        }, 0);
      document.getElementById('s-uncldesc').hidden =
        goalSelect.value !== 'unclassified';
      var goals = goalCache[gameSelect.value] || [];
      var picked = goals.filter(function (g) {
        return g.key === goalSelect.value;
      })[0];
      paintSub(picked);
    ...
    function draftFields() {
      ...
      Array.prototype.forEach.call(submitForm.elements, function (e) {
        ...
        if (e.name in out) {
          if (!Array.isArray(out[e.name])) out[e.name] = [out[e.name]];
          out[e.name].push(v);
        } else out[e.name] = v;
      });
      return out;
    ...
    function applyDraftFields(fields) {
      ...
      var names = [].concat(fields.file_name || []),
        shas = [].concat(fields.file_sha1 || []);
      if (rowsBox && names.length) {
        rowsBox.querySelectorAll('.filerow').forEach(function (r) {
          r.remove();
        });
        names.forEach(function () {
          rowsBox.querySelector('.addfile').click();
        });
      }
      var seen = {};
      Array.prototype.forEach.call(submitForm.elements, function (e) {
      ...
      if (rowsBox && names.length) {
        ...
        shas.forEach(function (v, i) {
          if (shaIns[i]) {
            shaIns[i].value = v;
            shaIns[i].dispatchEvent(new Event('input'));
          }
        });
      }
      paintKind();
      var pick = submitForm.querySelector('.authpick');
      if (fields.authors && pick && pick.setAuthors)
        pick.setAuthors(fields.authors.split(',').filter(Boolean));
    }
    function stamp(t) {
      var d = new Date(t);
      return (
    ...
    function saveDraft() {
      ...
      var typed =
        ...
        Object.keys(fields).some(function (k) {
          return (
            k !== 'authors' &&
            k !== 'goal_description' &&
            [].concat(fields[k]).some(function (v) {
              return v;
            })
          );
        });
      ...
      var data = {
        ...
        sub: subSelect.disabled ? '' : subSelect.value,
        fields: fields,
      };
      try {
    ...
    function enterEditMode() {
      ...
      fetch(api + '/api/run/record?run=' + encodeURIComponent(editRunId), {
        credentials: 'include',
      })
        .then(function (r) {
          return r.json();
        ...
        .then(function (j) {
          if (!j.ok) {
            byIdS('s-subtitle').textContent =
              j.error || 'could not load the run';
          ...
          if (!mayEdit) {
            byIdS('s-subtitle').textContent =
              'Only the authors of ' +
              editRunId +
              ', the experts covering ' +
              j.game.title +
              ' and editors may edit it.';
            submitForm.hidden = true;
            return;
          }
          byIdS('s-subtitle').textContent =
          ...
          pendingDraft = {
            game: j.game.key,
            goal: run.category.goal,
            sub: run.category.sub || '',
            fields: {},
          };
          pickGame(j.game.key);
          ...
          document.querySelectorAll('#p-game .createq').forEach(function (e) {
            e.hidden = true;
          });
          ...
          submitForm.querySelector('[name=encode]').value =
            ((run.encodes || [])[0] || {}).url || '';
          submitForm
            .querySelector('[name=encode]')
            .dispatchEvent(new Event('input'));
          var pick = submitForm.querySelector('.authpick');
          if (pick && pick.setAuthors)
            pick.setAuthors(
              run.authors.map(function (a) {
                return a.user;
              })
            );
          if (!editMay.author && !editMay.expert) {
            pick.querySelector('.authsearch').disabled = true;
            pick.querySelectorAll('.authx').forEach(function (x) {
              x.disabled = true;
            });
          }
          ...
          var relatedPick = armMultiPick(
            ...
            function (q) {
              return searchArchive('runs', q).then(function (items) {
                relatedList.innerHTML = '';
                items.forEach(function (it) {
                  if (it.value === editRunId) return;
                  if (relatedSeen.indexOf(it.value) < 0)
                    relatedSeen.push(it.value);
                  var o = document.createElement('option');
                  o.value = it.value;
          ...
          (run.related || []).forEach(function (v) {
            if (relatedSeen.indexOf(v) < 0) relatedSeen.push(v);
          });
          ...
          if (!run.movie) {
            byIdS('s-movielabel').textContent =
              'The movie file, if this run has one after ' +
              'all (optional; adding it stops the run being video-only)';
          } else if (expertish) {
            byIdS('s-movielabel').textContent =
              'Replace the movie file (optional; the current one stays unless you pick another)';
          } else {
            movieWrap.hidden = true;
          }
          var movieSec =
            run.movie && run.movie.frames && run.movie.fps
              ? run.movie.frames / run.movie.fps
              : j.seconds || null;
          movieInfo = run.videoOnly
            ? null
            : {
                parsed: !!(run.movie && run.movie.frames),
                frames: (run.movie || {}).frames,
                seconds: movieSec,
                format: (run.movie || {}).format,
              };
          submitForm.querySelector('[name=emulator]').value =
            (run.contract || {}).emulator || '';
          updateEmulatorAssistant(systemOf(j.game.key));
          syncPresetFromInput();
          var files =
            (run.contract || {}).files ||
            ((run.contract || {}).rom ? [run.contract.rom] : []);
          var rowsBox = submitForm.querySelector('.filerows');
          files.forEach(function (f) {
            rowsBox.querySelector('.addfile').click();
            var rows = rowsBox.querySelectorAll('.filerow');
            var row = rows[rows.length - 1];
            row.querySelector('[name=file_name]').value = f.name || '';
            row.querySelector('[name=file_sha1]').value = f.sha1 || '';
          });
          ...
          stagedAttachments = [];
          renderAttachments();
          (run.contentWarnings || []).forEach(function (w) {
            var b = submitForm.querySelector(
              '[name=content_warnings][value="' + w + '"]'
            );
            if (b) b.checked = true;
          });
          ...
          if (run.goalDescription)
            submitForm.querySelector('[name=goal_description]').value =
              run.goalDescription;
          if (!editMay.author) {
            byIdS('s-why').hidden = false;
            submitForm.querySelector('[name=reason]').required = true;
          }
          if (!editMay.author && !editMay.expert) {
            ...
            submitForm
              ...
              .forEach(function (e) {
                if (
                  ['goal', 'sub', 'reason'].indexOf(e.name) < 0 &&
                  e.id !== 's-goal' &&
                  e.id !== 's-sub'
                )
                  e.disabled = true;
            ...
            byIdS('s-subtitle').textContent +=
              ' · editor: category and subcategory only';
          }
          submitForm
            .querySelector('[name=consent]')
        ...
        .catch(function () {
          byIdS('s-subtitle').textContent = 'could not reach the archivist';
        });
    }
    function editStatedTime(run, sec) {
      ...
      if (!sec && run.movie && run.movie.frames && run.movie.fps)
        sec = run.movie.frames / run.movie.fps;
    ...
    var panels = Array.prototype.slice.call(
      submitForm.querySelectorAll('.panel')
    );
    var revealed = { 1: true };
    var previewed = false;
    ...
    function panelWhy(step) {
      ...
      if (step === 2)
        return document.getElementById('enc-status').className !== 'enc-good'
          ? 'the encode link is not valid'
          : 'name at least one author';
      if (step === 3)
        return 'the movie file could not be checked; pick it again or remove it';
      if (step === 4)
        return timeStatedNeeded() &&
          !/^(\d+:)?\d{1,2}:\d{2}/.test(document.getElementById('s-time').value)
          ? 'state the time'
          : 'every value the category ranks by is needed';
      if (step === 5) return 'preview your notes';
      if (step === 6)
        return editRunId
          ? 'say why, publicly (at least 8 characters)'
          : 'tick the agreement';
      return '';
    }
    var panelNames = {
    ...
    function panelDone(step) {
      ...
      if (step === 2) {
        return (
          document.getElementById('enc-status').className === 'enc-good' &&
      ...
      if (step === 4) {
        ...
        var ok = Array.prototype.every.call(
          submitForm.querySelectorAll('#s-mfields input[type=hidden]'),
          function (h) {
            return h.value !== '' && !isNaN(+h.value);
          }
        );
        if (
          timeStatedNeeded() &&
      ...
      if (step === 6)
        return editRunId
          ? editMay && editMay.author
            ? true
            : submitForm.querySelector('[name=reason]').value.trim().length >= 8
          : submitForm.querySelector('[name=consent]').checked;
      return false;
    }
    ...
    submitForm.addEventListener('change', function () {
      clearTimeout(draftTimer);
      draftTimer = setTimeout(saveDraft, 300);
    });
    document.getElementById('s-clear').addEventListener('click', function () {
      if (
        !window.confirm(
          editRunId
            ? 'Discard your changes and reload the archived record?'
            : 'Clear every field and discard the saved draft?'
        )
      )
        return;
    ...
    function restoreDraft() {
      ...
      if (!d || !d.t || Date.now() - Date.parse(d.t) > 30 * 86400000)
        return false;
      draftNote.hidden = false;
    ...
    var systemNames = {};
    try {
      systemNames = JSON.parse(
        document.getElementById('systemdata').textContent
      );
    } catch (e) {
      systemNames = {};
    }
    ...
    function fillSystems(selected) {
      ...
      var keys = Object.keys(systemNames).sort(function (a, b) {
        return systemNames[a].toLowerCase() < systemNames[b].toLowerCase()
          ? -1
          : 1;
      });
      ...
      every.value = '';
      every.textContent = 'Every system';
      systemSelect.appendChild(every);
      keys.forEach(function (k) {
        var o = document.createElement('option');
        o.value = k;
        o.textContent = systemNames[k];
    ...
    function fillGameList() {
      ...
      gameList.innerHTML = '';
      var pool = gameKeys.filter(function (k) {
        return !sys || systemOf(k) === sys;
      });
      ...
      var rows = q
        ? pool.filter(function (k) {
            return (
              gameTitles[k].toLowerCase().indexOf(q) >= 0 || k.indexOf(q) >= 0
            );
          })
        : sys
          ? pool
      ...
      if (sys && !rows.length) {
        gameList.appendChild(
          el(
            ...
            'authopt authnone',
            q
              ? 'no game of that name on this system yet'
    ...
    if (addSystemBtn) {
      ...
      newGo.addEventListener('click', function () {
        ...
        if (name.length < 2) {
          note(newMsg, 'the name, as people write it', false);
          return;
        }
        var fd = new FormData();
        fd.append('name', name);
        post('/api/system/create', fd, newGo).then(function (res) {
          if (res.ok && res.j.ok) {
            ...
            note(
              newMsg,
              res.j.system.name +
                ' is a system here now, as ' +
                res.j.key +
                '. Create the game on it, then come back.',
              true
            );
            newName.value = '';
            fillGameList();
    ...
    var presetGame = null;
    try {
      presetGame = new URLSearchParams(location.search).get('game');
    } catch (e) {}
    ...
    if (!editRunId && presetGame && gameTitles[presetGame]) {
      ...
      gamePick.hidden = true;
      gameLocked.hidden = false;
      document.getElementById('s-gamelockname').textContent =
        gameTitles[presetGame];
      document
        .getElementById('s-gameunlock')
    ...
    if (editRunId)
      setTimeout(function () {
        enterEditMode();
      }, 0); // once every handler below is wired
    ...
    function checkEncode() {
      ...
      fetch(api + '/api/encode/check?url=' + encodeURIComponent(url), {
        ...
        .then(function (j) {
          ...
          if (!j.ok) {
            encodeStatus.textContent =
              '✗ ' + (j.error || 'that link does not work');
            encodeStatus.className = 'enc-bad';
            paintPanels();
            return;
          }
          ...
          encodeSeconds =
            typeof j.seconds === 'number' && j.seconds > 0 ? j.seconds : null;
          paintTimeImport();
          paintPanels();
          if (j.thumb) {
            encodeThumb.onerror = function () {
              encodeThumb.hidden = true;
            };
            encodeThumb.onload = function () {
              encodeThumb.hidden = false;
            };
        ...
        .catch(function () {
          encodeStatus.textContent =
            '✗ could not reach the archivist to check the link';
          encodeStatus.className = 'enc-bad';
          paintPanels();
        });
    }
    ...
    document
      ...
      .addEventListener('click', function () {
        ...
        var goalLabel =
          goalSelect.value === 'unclassified'
            ? 'Unclassified'
            : goalSelect.options[goalSelect.selectedIndex]
        ...
        document.getElementById('pv-chips').innerHTML =
          ...
          '</span><span class="chip pendchip">Pending</span>';
        ...
        previewForm.append(
          'notes',
          submitForm.querySelector('[name=notes]').value || ''
        );
        fetch(api + '/api/preview', {
          ...
          .then(function (j) {
            previewNotes.innerHTML = j.ok
              ? j.html
              : escapeHtml(j.error || 'preview failed');
          })
          .catch(function () {
            previewNotes.textContent =
              'The archivist is not reachable; the preview needs it.';
          });
    ...
    function saveEdit() {
      var submitBtn = document.getElementById('s-submit');
      var run = editRecord.run,
        expertish = editMay.expert || editMay.editor;
      function revision(dry) {
        ...
        [
          ...
          var e = submitForm.querySelector('[name=' + n + ']');
          if (e)
            fd.append(
              n === 'goal_description' ? 'goalDescription' : n,
              e.value
            );
        });
        if (editMay.author)
        ...
        submitForm
          .querySelectorAll('[name=content_warnings]:checked')
          .forEach(function (c) {
            fd.append('content_warnings', c.value);
          });
        fd.append('files_set', '1');
        submitForm.querySelectorAll('[name=file_name]').forEach(function (i) {
          fd.append('file_name', i.value);
        });
        submitForm.querySelectorAll('[name=file_sha1]').forEach(function (i) {
          fd.append('file_sha1', i.value);
        });
        submitForm
          .querySelectorAll('input[name^=metric_]')
          .forEach(function (h) {
            fd.append(h.name, h.value);
          });
        if (timeStatedNeeded()) fd.append('time', byIdS('s-time').value);
        if (pickedMovie && (editMay.author || editMay.expert))
          fd.append('movie', pickedMovie);
        if (editMay.author) {
      ...
      var newGoal = expertish
        ? goalSelect.value + (subWrap.hidden ? '' : '/' + subSelect.value)
        : null;
      var oldGoal =
        run.category.goal + (run.category.sub ? '/' + run.category.sub : '');
      ...
      var moveGame =
        editMay.expert && gameSelect.value && gameSelect.value !== run.game
          ? gameSelect.value
          : null;
      var moveGoal =
        !moveGame && newGoal && newGoal !== oldGoal ? newGoal : null;
      var liveVerifications = (run.verifications || []).filter(function (v) {
      ...
      var dry = mayRevise
        ? post('/api/edit', revision(true), submitBtn)
      ...
      dry.then(function (res) {
        ...
        if (!(res.ok && res.j.ok) && !nothing) {
          if (res.j.error && res.j.error.indexOf('attachment') !== -1)
            setAttError(res.j.error);
          note(msg, res.j.error || 'something went wrong', false);
          return;
        }
        if ((moveGame || moveGoal) && liveVerifications)
          wv = wv.concat(['verifications']);
        var text = '';
        if (wv.indexOf('verifications') >= 0)
          text =
            'This run is verified. Changing its scoring invalidates the verifications: it leaves the ranking until somebody verifies it again.';
        if (wv.indexOf('reproductions') >= 0)
          text +=
            (text ? ' ' : '') +
        ...
        if (!nothing)
          steps.push(function () {
            return post('/api/edit', revision(false), submitBtn);
          });
        if (moveGame)
          steps.push(function () {
            var fd = new FormData();
            fd.append('run', editRunId);
            fd.append('game', moveGame);
            fd.append('goal', goalSelect.value);
            if (!subWrap.hidden && subSelect.value)
              fd.append('sub', subSelect.value);
            fd.append(
              'reason',
              submitForm.querySelector('[name=reason]').value.trim() ||
        ...
        if (moveGoal)
          steps.push(function () {
            var fd = new FormData();
            fd.append('kind', 'run');
            fd.append('target', editRunId);
            fd.append('field', 'goal');
            fd.append('value', moveGoal);
            fd.append(
              'reason',
              submitForm.querySelector('[name=reason]').value.trim() ||
        ...
        if (!steps.length) {
          submitting = false;
          note(msg, 'Nothing changed.', false);
          return;
        }
        var serial = null,
          voided = [];
        (function step() {
          if (!steps.length) {
            ...
            var editRunLines = [
              'Saved.' +
                (voided.length
                  ? ' Invalidated: ' + voided.join(', ') + '.'
                  : ''),
              'The run page shows it now: <a href="' +
                escapeHtml(editRunUrl) +
                '">' +
                escapeHtml(editRunUrl) +
                '</a>.',
            ];
          ...
          steps
            ...
            .then(function (r2) {
              if (r2.ok && r2.j.ok) {
                serial = r2.j.serial || serial;
                (r2.j.voided || []).forEach(function (v) {
                  if (voided.indexOf(v) < 0) voided.push(v);
                });
                step();
              } else {
                submitting = false;
                if (
                  r2.j &&
                  r2.j.error &&
                  r2.j.error.indexOf('attachment') !== -1
                )
                  setAttError(r2.j.error);
                note(
                  msg,
                  (r2.j && r2.j.error) || 'something went wrong',
                  false
                );
              }
            });
      ...
      post('/api/submit', fd, submitBtn).then(function (res) {
        ...
        if (res.ok && res.j.ok) {
          ...
          var lines = [
            ...
            forumUrl
              ? 'Announced on the forum: <a href="' +
                escapeHtml(forumUrl) +
                '">' +
                escapeHtml(forumUrl) +
                '</a>'
              : '',
            'Your run page is live at <a href="' +
              escapeHtml(runUrl) +
              '">' +
              escapeHtml(runUrl) +
              '</a>.',
          ...
          if (res.j && res.j.error && res.j.error.indexOf('attachment') !== -1)
            setAttError(res.j.error);
