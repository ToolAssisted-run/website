...
function mountClaimsBoard(ids, msg) {
      ...
      res.j.pending.forEach(function (claim) {
        ...
        line.appendChild(
          el(
            'span',
            'actmeta',
            ' ' + (claim.email || 'no address on file') + ' · ' + claim.date
          )
        );
  ...
  document.getElementById(ids.yes).addEventListener('click', function () {
    answer(true);
  });
...
if (founderPanelEl) {
  ...
  mePromise.then(function (d) {
    ...
    if (d.unreachable) {
      gate.textContent =
        'The archivist is unreachable, so who you are cannot be ' +
        'checked right now.';
      return;
    }
    if (
      !d.loggedIn ||
      viewAsActive() ||
      founderData.founders.indexOf(d.user.toLowerCase()) < 0
    ) {
    ...
    var seated = (founderData.committee || []).map(function (x) {
      return x.toLowerCase();
    });
    armPicker(document.querySelector('#f-seat [name=target]'), {
      source: function (q) {
        return searchArchive('members', q);
      },
      filter: function (it) {
        return seated.indexOf(it.value.toLowerCase()) < 0;
      },
      placeholder: 'type to find a member',
      empty: 'no such member, or already seated',
    });
      ...
      form.addEventListener('submit', function (ev) {
        ...
        post(
          ...
          if (res.ok && res.j.ok) {
            noteBuilt(
              msg,
              (res.j.action === 'granted' ? 'Seated. ' : 'Unseated. ') +
                (res.j.told || ''),
              res.j.serial
            );
            form.reset();
...
armSystemForm('f-system', 'system-msg', '/api/system/create', function (j) {
  return (
    j.system.name + ' is a system here now, as ' + j.key + '. ' + (j.note || '')
  );
});
armSystemForm(
  'f-systemedit',
...
if (committeePanelEl) {
  ...
  mePromise.then(function (d) {
    ...
    if (!d.loggedIn) {
      gate.innerHTML =
        'This panel is for the Steering Committee. <a href="' +
        api +
    ...
    if (
      viewAsActive() ||
      committeeData.committee.indexOf(d.user.toLowerCase()) < 0
    ) {
      gate.textContent =
        'This panel is for the Steering Committee, and you are not on it.';
      return;
    }
    gate.hidden = true;
    ...
    mountClaimsBoard(
      {
        list: 'cpanel-list',
        form: 'f-claimdecide',
        identity: 'cdecide-identity',
        what: 'cdecide-what',
        yes: 'cdecide-yes',
        no: 'cdecide-no',
        cancel: 'cdecide-cancel',
      },
      committeeMsg
    );
    ...
    function roleHolders() {
      return (
        roleSelect.value === 'committee'
          ? committeeData.committeeNames
          : roleSelect.value === 'editor'
            ? committeeData.editors || []
    ...
    var rolePicker = armPicker(roleForm.querySelector('[name=target]'), {
      ...
      placeholder: 'type to find a member',
      empty: 'nobody matches for this role and decision',
    });
        ...
        if (res.ok && res.j.ok) {
          noteBuilt(
            ...
            'Recorded: ' +
              res.j.votes +
              ' of the ' +
              (res.j.cast || res.j.committee) +
              ' votes cast went for it.',
            res.j.serial
          );
          roleForm.reset();
    ...
    var me = d.user.toLowerCase(),
      founders = (committeeData.founders || []).map(function (x) {
        return x.toLowerCase();
      });
    var seatedNames = committeeData.committeeNames.map(function (x) {
      return x.toLowerCase();
    });
    armPicker(deleteForm.querySelector('[name=target]'), {
      ...
      placeholder: 'type to find a member',
      empty: 'no such member, or not yours to delete',
    });
    deleteForm.addEventListener('submit', function (ev) {
      ...
      if (
        !window.confirm(
          'Delete the member ' + target + ' outright? This cannot be undone.'
        )
      )
        return;
      post(
        ...
        if (res.ok && res.j.ok) {
          noteBuilt(
            ...
            'Deleted.',
            res.j.serial,
            'Their page is gone from the site now.'
          );
          deleteForm.reset();
    ...
    armPicker(document.getElementById('siteexpert-user'), {
      ...
      filter: function (it) {
        return committeeData.siteExperts.indexOf(it.value.toLowerCase()) < 0;
      },
      placeholder: 'type to find a member',
      empty: 'no such member, or already a whole-site expert',
    });
    ...
    siteExpertForm.addEventListener('submit', function (ev) {
      ...
      post(
        '/api/expert/appoint',
        new FormData(siteExpertForm),
        actionBtn(siteExpertForm)
      ).then(function (res) {
        if (res.ok && res.j.ok) {
          noteBuilt(
            committeeMsg,
            res.j.user + ' is now an expert for the whole site.',
            res.j.serial
          );
    ...
    var editorNames = (committeeData.editors || []).map(function (x) {
      return x.toLowerCase();
    });
    armPicker(document.getElementById('editorrole-user'), {
      ...
      filter: function (it) {
        return editorNames.indexOf(it.value.toLowerCase()) < 0;
      },
      placeholder: 'type to find a member',
      empty: 'no such member, or already an editor',
    });
    ...
    editorForm.addEventListener('submit', function (ev) {
      ...
      post(
        ...
        actionBtn(editorForm)
      ).then(function (res) {
        if (res.ok && res.j.ok) {
          noteBuilt(
            committeeMsg,
            res.j.user + ' is now an editor.',
            res.j.serial
          );
          editorForm.reset();
        } else note(committeeMsg, res.j.error || 'something went wrong', false);
...
if (expertPanelEl) {
  ...
  mePromise.then(function (d) {
    ...
    if (!d.loggedIn) {
      gate.innerHTML =
        'This panel is for experts. <a href="' +
        api +
        '/login">Log in</a> to see whether it is yours.';
      return;
    }
    var myName = d.user.toLowerCase();
    var myRoster = panelData.roster.filter(function (e) {
      return e.user.toLowerCase() === myName;
    });
    ...
    if (!myRoster.length && !amCommittee) {
      gate.textContent =
        'This panel is for experts, and you hold no scope. Experts ' +
    ...
    myRoster.forEach(function (e) {
      ...
      line.appendChild(
        el(
          'span',
          'actmeta',
          ' since ' + e.date + (e.by ? ', appointed by ' + e.by : '')
        )
      );
    ...
    panelData.groups.forEach(function (gr) {
      ...
      (gr.games || []).forEach(function (k) {
        groupOf[k] = gr.key;
      });
    });
    function coversGame(scopes, key) {
      return (
        scopes.indexOf('site') >= 0 ||
        scopes.indexOf(key) >= 0 ||
        scopes.indexOf(key.split('/')[0]) >= 0 ||
        (groupOf[key] && scopes.indexOf('group:' + groupOf[key]) >= 0)
      );
    }
    function coversGroup(scopes, key) {
      if (scopes.indexOf('site') >= 0 || scopes.indexOf('group:' + key) >= 0)
        return true;
      var inside = gamesIn[key] || [];
      return (
        inside.length > 0 &&
        inside.every(function (k) {
          return coversGame(scopes, k);
        })
      );
    }
    function scopesOf(name) {
      var low = name.toLowerCase();
      return panelData.roster
        .filter(function (e) {
          return e.user.toLowerCase() === low;
        })
        .map(function (e) {
          return e.scope;
        });
    }
    var myScopes = myRoster.map(function (e) {
      return e.scope;
    });
    ...
    var myGroups = panelData.groups.filter(function (gr) {
      return coversGroup(myScopes, gr.key);
    });
    ...
    var gamePicker = armPicker(document.getElementById('appoint-game'), {
      source: function (q) {
        return searchArchive('games', q);
      },
      filter: function (it) {
        return amCommittee || coversGame(myScopes, it.value);
      },
      placeholder: 'type to find a game',
      empty: 'no such game, or not yours to hand out',
      onPick: function () {
        refreshCandidates(gamePicker.input, userPickers['appoint-game-user']);
      },
    });
    fillSelect(
      document.getElementById('appoint-group'),
      apptGroups.map(function (gr) {
        return { value: 'group:' + gr.key, label: gr.title };
      }),
    ...
    if (amSite || amCommittee) {
      ...
      fillSelect(
        document.getElementById('appoint-wide'),
        panelData.scopes
          .filter(function (s) {
            return (
              s.key !== 'site' &&
    ...
    [
      ['appoint-game', 'appoint-game-user'],
      ['appoint-group', 'appoint-group-user'],
      ['appoint-wide', 'appoint-wide-user'],
      ...
      userPickers[pair[1]] = armPicker(userField, {
        ...
        filter: function (it) {
          var sc = scopeOf();
          return !!sc && !speaksFor(it.value, sc);
        },
        placeholder: 'type to find a member',
        empty:
          'pick the scope first, or everybody matching already speaks for it',
      });
      var scopeSelect = document.getElementById(pair[0]);
      if (scopeSelect && scopeSelect.tagName === 'SELECT') {
        scopeSelect.addEventListener('change', function () {
          refreshCandidates(scopeSelect, userPickers[pair[1]]);
        });
      }
    });
    ...
    fillSelect(
      document.getElementById('resign-scope'),
      [{ value: '', label: 'every scope I hold' }].concat(
        myRoster.map(function (e) {
          return { value: e.scope, label: e.label };
        })
      ),
      'nothing to step down from'
    );
    fillSelect(
      document.getElementById('groupedit-key'),
      myGroups.map(function (gr) {
        return { value: gr.key, label: gr.title };
      }),
    ...
    function groupableFill(q) {
      return searchArchive('games', q).then(function (items) {
        ...
        items
          .filter(function (it) {
            return it.item && !it.item.group && coversGame(myScopes, it.value);
          })
          .forEach(function (it) {
            if (groupableKeys.indexOf(it.value) < 0)
              groupableKeys.push(it.value);
            var o = document.createElement('option');
            o.value = it.value;
    ...
    armMultiPick(
      document.getElementById('f-groupnew'),
      'games',
      'panel-gamelist',
      function () {
        return groupableKeys;
      },
      groupableFill
    );
    var groupEditSelect = document.getElementById('groupedit-key');
    armMultiPick(
      document.getElementById('f-groupedit'),
      'add',
      'panel-gamelist',
      function () {
        return groupableKeys;
    ...
    ['f-appoint-game', 'f-appoint-group', 'f-appoint-wide'].forEach(
      function (id) {
