...
if (impMsg && api) {
  ...
  function importPost(path, fd, btn) {
    ...
    return fetch(api + path, {
      ...
      .catch(function () {
        return {
          ok: false,
          error: 'network error; the archivist may be unreachable',
        };
      })
      .then(function (r) {
        busy(btn, false);
  ...
  soloBtn.addEventListener('click', function () {
    boxes.forEach(function (b) {
      b.box.checked = !b.multi && !b.box.disabled;
    });
    paintRun();
  });
  ...
  scanBtn.addEventListener('click', function () {
    ...
    importPost('/api/import/scan', undefined, scanBtn).then(function (d) {
      if (!d.ok) {
        note(impMsg, d.error || 'scan failed', false);
        return;
      }
      impMsg.hidden = true;
      ...
      try {
        d.pending.forEach(function (x) {
          titles['M' + x.id] = x.title;
        });
        if (!d.pending.length) {
          scanline.textContent =
            'TASVideos: nothing to import, all ' +
            d.total +
            ' of your publications there are already archived (snapshot of ' +
            d.backupDate +
            ').';
        } else {
          scanline.textContent =
            ...
            ' of your ' +
            d.total +
            ' publications there are not archived yet (snapshot of ' +
            d.backupDate +
        ...
        d.pending.forEach(function (x) {
          ...
          if (x.multiAuthor && !blocked) {
            var others = x.authors
              .filter(function (a) {
                return a.toLowerCase() !== (d.user || '').toLowerCase();
              })
              .join(', ');
            row.appendChild(
              el(
                ...
                ' co-authored with ' +
                  others +
                  '; importing it is your responsibility'
              )
            );
          }
    ...
    function step() {
      importPost('/api/import/run', fd).then(function (r) {
        if (!r.ok) {
          ...
          note(
            ...
            'Import stopped after ' +
              ...
              ' of ' +
              total +
              '; your progress is saved. Reload to resume.',
            false
          );
          impMsg.hidden = false;
          busy(runBtn, false);
        ...
        progressCount.textContent =
          Math.min(done + skips, total) + ' / ' + total;
        progressFill.style.width =
          Math.round((100 * Math.min(done + skips, total)) / total) + '%';
        if (r.remaining > 0 && r.imported.length) {
          step();
          return;
        }
        logLine('');
        logLine(
          'Done: ' +
            ...
            ' imported' +
            (skips ? ', ' + skips + ' need attention (see above)' : '') +
            '.'
        );
        ...
        waitBuilt(r.serial, function (live) {
          logLine(
            live
              ? 'Your runs are live on the site now.'
