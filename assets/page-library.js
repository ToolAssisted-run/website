...
function armZone(dataId, zoneId, msgId, forms) {
  ...
  mePromise.then(function (d) {
    ...
    var isExpertHere =
      viewAsCoverage(zoneData.experts || zoneData.siteExperts, who).indexOf(
        who
      ) >= 0;
    ...
    if (!isExpertHere && isEditorHere) {
      ...
      if (zoneHeading && /Expert menu/.test(zoneHeading.textContent))
        zoneHeading.textContent = 'Editor menu';
    }
      ...
      if (zoneData.movable) {
        var moveRows = zoneData.movable.map(function (g) {
          ...
          checkbox.addEventListener('change', function () {
            if (checkbox.checked) picked.add(g.key);
            else picked.delete(g.key);
            syncMoveField();
          });
        ...
        if (moveFilter)
          moveFilter.addEventListener('input', function () {
            ...
            moveRows.forEach(function (r) {
              r.hidden = !!n && r.dataset.hay.indexOf(n) === -1;
            });
          });
          ...
          debTimer = setTimeout(function () {
            searchArchive('games', q).then(function (items) {
              ...
              checkedEls.forEach(function (cb) {
                kept[cb.value] = cb.closest('label');
              });
              moveList.innerHTML = '';
              Object.values(kept).forEach(function (el) {
              ...
              items.forEach(function (item) {
                ...
                checkbox.addEventListener('change', function () {
                  if (checkbox.checked) picked.add(item.value);
    ...
    forms.forEach(function (spec) {
      ...
      form.addEventListener('submit', function (ev) {
        ...
        post(spec.path, new FormData(form), actionBtn(form)).then(
          function (res) {
            if (res.ok && res.j.ok) {
...
armZone('gameactdata', 'f-gameremove-wrap', 'gameact-msg', [
  {
    id: 'f-gamerename',
    path: '/api/expert/edit',
    done: function (j) {
      return 'Renamed to ' + j.to + '.';
    },
  },
  {
    id: 'f-gamethumb',
  ...
  },
  {
    ...
    expertOnly: true,
    confirm:
      'Delete this game outright, WITH every run in it? ' +
      'This cannot be undone.',
    done: function (j) {
      return (
        'Deleted. ' +
        (j.runs_deleted && j.runs_deleted.length
          ? j.runs_deleted.length + ' run(s) deleted with it.'
          : 'It held no runs.')
      );
    },
  },
...
armZone('groupactdata', 'groupacts', 'groupact-msg', [
  ...
  },
  {
    ...
    confirm:
      'Delete this group outright? Its games become ungrouped. ' +
      'This cannot be undone.',
    done: function (j) {
      return 'Deleted; ' + j.released.length + ' game(s) are ungrouped.';
    },
  },
]);
armZone('gamesactdata', 'gamesacts', 'gamesact-msg', [
  {
    ...
    path: '/api/group/create',
      ...
      return 'The ' + j.group + ' group exists.';
    },
  },
]);
