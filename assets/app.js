...
export var mePromise = fetch(api + '/api/me', { credentials: 'include' })
  ...
  .catch(function () {
    return { loggedIn: false, unreachable: true };
  });
...
function viewAsMode() {
  try {
    return sessionStorage.getItem('tar-viewas') || '';
  } catch (e) {
    return '';
  }
}
var viewAsHonored = false; // the wrap below decides whether the key is honored
export function viewAsActive() {
  return viewAsHonored ? viewAsMode() : '';
}
...
mePromise = mePromise.then(function (d) {
  ...
  if (!m) return d;
  if (!T.viewasEligible) {
    // a stale key on lesser accounts is noise
    try {
      sessionStorage.removeItem('tar-viewas');
    } catch (e) {}
    return d;
  }
  viewAsHonored = true;
  ...
  ['experts', 'editors', 'committee', 'founders'].forEach(function (k) {
    T[k] = (T[k] || []).filter(function (n) {
      return n.toLowerCase() !== who;
    });
  });
...
export function setLastBtn(btn) {
  lastBtn = btn;
}
...
export function noteBuilt(box, doneText, serial, liveText, keepText) {
  var btn = lastBtn;
  var onMark =
    btn && setMark(btn, 'spin', doneText + ' Publishing to the site…');
  if (!onMark || keepText)
    noteText(box, doneText + ' Publishing to the site…', true);
  else if (box) box.hidden = true;
  waitBuilt(serial, function (live) {
    var tail = live
      ? liveText || 'It is live on the site now.'
...
document.querySelectorAll('input.mirror').forEach(function (box) {
  var hidden =
    box.form && box.form.querySelector('input[type=hidden][name=value]');
  if (!hidden) return;
...
export function post(path, fd, btn) {
  ...
  return fetch(api + path, { method: 'POST', body: fd, credentials: 'include' })
    ...
    .catch(function () {
      return {
        ok: false,
        j: { error: 'network error; the archivist may be unreachable' },
      };
    })
    .then(function (res) {
      busy(btn, false);
      ...
      if (btn)
        setMark(
          btn,
          res.ok && res.j && res.j.ok ? 'ok' : 'bad',
          res.ok && res.j && res.j.ok
            ? 'Done'
            : (res.j && res.j.error) || 'something went wrong'
        );
      return res;
    });
}
...
function setTheme(t) {
  try {
    if (t === 'system') {
      localStorage.removeItem('tar-theme');
      delete document.documentElement.dataset.theme;
    } else {
      localStorage.setItem('tar-theme', t);
...
mePromise.then(function (d) {
  ...
  if (offlineNote && d.unreachable) {
    ...
    offlineNote.addEventListener('click', function () {
      ...
      fetch(api + '/api/me', { credentials: 'include' })
        ...
        .catch(function () {
          offlineNote.textContent = 'still unreachable';
        });
    });
  }
  ...
  var profileHref =
    rel + 'authors/' + encodeURIComponent(d.user.toLowerCase()) + '/';
  var bell = el('a', 'bell');
  ...
  if (location.pathname.endsWith('/authors/' + slug + '/')) {
    try {
      localStorage.setItem(seenKey, new Date().toISOString().slice(0, 10));
    } catch (e) {}
  } else {
    fetch(rel + 'assets/news.json' + versionQuery)
      .then(function (r) {
        return r.json();
      })
      .then(function (all) {
        ...
        var fresh = dates.filter(function (dt) {
          return dt > seen;
        }).length;
        if (fresh > 0) {
  ...
  if (d.avatar) {
    ...
    avatarBtn.appendChild(
      el('span', 'avatar avatar-fallback', d.user.charAt(0).toUpperCase())
    );
  }
  ...
  fetch(rel + 'assets/authorstats.json' + versionQuery)
    .then(function (r) {
      return r.json();
    })
    .then(function (all) {
      ...
      [
        [String(s.runs), 'runs', ''],
        ['<span class="starglyph">★</span>' + s.author, 'author score', ''],
  ...
  menu.appendChild(
    item('Account settings', 'https://forum.toolassisted.run/my/preferences')
  );
  ...
  if (T.viewasEligible) {
    ...
    viewAsSelect.addEventListener('click', function (ev) {
      ev.stopPropagation();
    });
    viewAsSelect.addEventListener('change', function () {
      try {
        if (viewAsSelect.value)
          sessionStorage.setItem('tar-viewas', viewAsSelect.value);
  ...
  try {
    currentTheme = localStorage.getItem('tar-theme') || 'system';
  } catch (e) {}
...
function localSource(items) {
  return function (q) {
    ...
    return items.filter(function (it) {
      return (
        it.label.toLowerCase().indexOf(low) >= 0 ||
        String(it.value).toLowerCase().indexOf(low) >= 0
      );
    });
  };
}
export function armPicker(field, opts) {
  ...
  function show(items) {
    ...
    items = items
      .filter(function (it) {
        return !opts.filter || opts.filter(it);
      })
      .slice(0, 12);
    if (!items.length) {
      list.appendChild(
        el('div', 'authopt authnone', opts.empty || 'nothing matches')
      );
    }
  ...
  function lookup() {
    ...
    if (picked && search.value !== picked.label) {
      picked = null;
      hidden.value = '';
    }
    if (!q) {
    ...
    var mine = ++serial;
    Promise.resolve(opts.source(q)).then(function (items) {
      if (mine === serial) show(items);
    });
  }
...
function initFileRows(box) {
  ...
  function addRow(entry) {
    ...
    picker.addEventListener('change', function () {
      ...
      sha1Hex(file)
        .then(function (hex) {
          shaIn.value = hex;
          shaIn.placeholder = 'SHA1 (40 hex), optional';
          checkSha();
        })
        .catch(function () {
          shaIn.placeholder = 'could not hash; type the SHA1';
        });
    });
    function checkSha() {
      ...
      shaIn.title = ok
        ? ''
        : 'a SHA1 is exactly 40 hexadecimal characters (' +
          v.length +
          ' so far)';
      return ok;
    }
    shaIn.addEventListener('input', checkSha);
    row.querySelector('.rmfile').addEventListener('click', function () {
      row.remove();
    });
    list.appendChild(row);
    return row;
  ...
  var seed = [];
  try {
    seed = JSON.parse(box.dataset.files || '[]');
  } catch (e) {}
  seed.forEach(addRow);
...
function cellValue(td) {
  ...
  var primaryText =
    mainChild && td.children && td.children[0] === mainChild
...
function compareCells(aTd, bTd, asc) {
  ...
  if (a.type === 'num' && b.type === 'num') {
    ...
    res = aStr.localeCompare(bStr, undefined, {
      numeric: true,
      sensitivity: 'base',
    });
  }
...
export function armSortableTable(table) {
  ...
  var wrap =
    (typeof table.closest === 'function' ? table.closest('.tblwrap') : null) ||
    table;
  var key =
    (table.dataset && table.dataset.pagesizeKey) ||
    (wrap && wrap.dataset && wrap.dataset.pagesizeKey);
  if (
    (table.classList &&
      typeof table.classList.contains === 'function' &&
    ...
    var itemLabel =
      (table.dataset && table.dataset.itemLabel) ||
      (wrap && wrap.dataset && wrap.dataset.itemLabel) ||
      'row';
    table._paginator = paginateElements(
      function () {
...
export function createPaginator(opts) {
  ...
  function renderControls() {
    ...
    var pageIndicator = el(
      'span',
      'pag-cur',
      'Page ' + currentPage + ' of ' + maxP
    );
      ...
      .then(function (data) {
        ...
        var groups = (data.groups || [])
          .map(function (gr) {
            ...
            return {
              item: gr,
              score: Math.max(sTitle, sMembers * 0.75, sKey * 0.8),
            };
          })
          .filter(function (x) {
            return x.score > 0;
        ...
        var games = (data.games || [])
          .map(function (g) {
            ...
            return {
              item: g,
              score: Math.max(sTitle, sGroups * 0.75, sKey * 0.8),
            };
          })
          .filter(function (x) {
            return x.score > 0;
        ...
        if (total === 0) {
          pop.innerHTML =
            '<div class="navsearch-empty">No results for "' +
            escapeHtml(q) +
            '"</div>';
          pop.hidden = false;
          activeIdx = -1;
        ...
        if (systems.length) {
          html +=
            '<div class="navsearch-group"><div class="navsearch-cat">Systems</div>';
          systems.forEach(function (sys) {
            html +=
              '<a class="navsearch-item" href="' +
              (rel || '') +
              'systems/' +
              escapeHtml(sys.k) +
              '/">' +
              '<span class="navsearch-badge">System</span>' +
              '<span class="navsearch-title">' +
              escapeHtml(sys.n) +
              '</span>' +
              '<span class="navsearch-sub">' +
              (sys.gc || 0) +
              ' game' +
              (sys.gc === 1 ? '' : 's') +
              ' · ' +
        ...
        if (groups.length) {
          html +=
            '<div class="navsearch-group"><div class="navsearch-cat">Game Groups</div>';
        ...
        if (games.length) {
          html +=
            '<div class="navsearch-group"><div class="navsearch-cat">Games</div>';
          games.forEach(function (g) {
            var grp =
              g.g && g.g.length ? ' · part of ' + escapeHtml(g.g[0]) : '';
            html +=
              '<a class="navsearch-item" href="' +
              (rel || '') +
              'games/' +
        ...
        if (authors.length) {
          html +=
            '<div class="navsearch-group"><div class="navsearch-cat">Authors</div>';
        ...
        if (runs.length) {
          html +=
            '<div class="navsearch-group"><div class="navsearch-cat">Runs</div>';
        ...
        html +=
          '<div class="navsearch-foot"><a class="navsearch-all" href="' +
          (rel || '') +
          'search/?q=' +
          encodeURIComponent(q) +
          '">View all matching results →</a></div>';
