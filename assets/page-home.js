...
document.querySelectorAll('.hrow[data-shuffle]').forEach(function (row) {
  ...
  cards.forEach(function (card, idx) {
    if (idx < keep) row.appendChild(card);
    else card.remove();
  });
});
...
function bskyPostHtml(item, profileUrl, linkify, since) {
  var postView = item.post || {},
    record = postView.record || {};
  var rkey = String(postView.uri || '')
    .split('/')
    .pop();
  ...
  if (embed.external && embed.external.uri) {
    card =
      '<a class="bcard" href="' +
      escapeHtml(embed.external.uri) +
      '" rel="noopener">' +
      (embed.external.thumb
        ? '<img src="' +
          escapeHtml(embed.external.thumb) +
          '" alt="" loading="lazy">'
        : '') +
      '<span><b>' +
      escapeHtml(embed.external.title || embed.external.uri) +
      '</b>' +
      escapeHtml((embed.external.description || '').slice(0, 90)) +
      '</span></a>';
    ...
    card =
      ...
      '" rel="noopener">' +
      '<img src="' +
      escapeHtml(embed.images[0].thumb) +
      '" alt="' +
      escapeHtml(embed.images[0].alt || '') +
      '" loading="lazy">' +
      '<span><b>Image</b>view on Bluesky</span></a>';
  }
  return (
    '<article class="bpost"><div class="btext">' +
    linkify(record.text || '') +
    '</div>' +
    card +
    '<div class="bmeta"><a href="' +
    escapeHtml(url) +
    '" rel="noopener">' +
    since(record.createdAt || postView.indexedAt) +
    '</a>' +
    '<span>♥ ' +
...
if (feedBox) {
  ...
  var showFeedError = function () {
    feedBox.innerHTML =
      '<p class="emptynote">Could not reach Bluesky just now. ' +
      'Read the latest at <a href="' +
      profileUrl +
      '">@' +
      escapeHtml(handle) +
      '</a>.</p>';
  ...
  fetch(
    ...
    .then(function (d) {
      ...
      if (!items.length) {
        feedBox.innerHTML =
          '<p class="emptynote">No posts yet. Follow ' +
          '<a href="' +
          profileUrl +
          '">@' +
          escapeHtml(handle) +
