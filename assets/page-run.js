...
if (actDataEl) {
  ...
  if (api) {
    ...
    fetch(api + '/api/visit', { method: 'POST', body: visitForm })
      ...
      .then(function (j) {
        if (!j.ok) return;
        document.getElementById('visitnum').textContent =
          j.visits.toLocaleString();
  ...
  mePromise.then(function (d) {
    ...
    var isExpert =
      myName !== null &&
      viewAsCoverage(runData.experts, myName).indexOf(myName) >= 0;
    var anything = false;
          ...
          post(path, fd, actionBtn(form)).then(function (res) {
            if (res.ok && res.j.ok) {
              var voided = (res.j.voided || []).length
                ? ' Its ' +
                  res.j.voided.join(' and ') +
                  ' were invalidated by this change.'
                : '';
              ...
              if (det) {
                ...
                if (sum && !sum.querySelector('.sumdone'))
                  sum.appendChild(el('span', 'sumdone', '\u2713 recorded'));
              }
              noteBuilt(
                msgBox,
                'Recorded, thank you: your act is archived.' +
                  voided +
                  ' You can leave this page whenever you like.',
                res.j.serial,
                null,
                true
              );
    ...
    var isEditor =
      myName !== null &&
      ((window.TAR || {}).editors || []).indexOf(myName) >= 0;
    if (isExpert || isEditor) {
      ...
      arm('f-move', '/api/expert/edit', function (form) {
        var goalSel = document.getElementById('mv-goal'),
          subSel = document.getElementById('mv-sub');
        var subWrap = document.getElementById('mv-subwrap'),
          valueField = form.querySelector('[name=value]');
        ...
        uncl.textContent = 'Unclassified';
        goalSel.appendChild(uncl);
        function paintMove() {
          var picked = runData.categories.filter(function (o) {
            return o.key === goalSel.value;
          })[0];
          ...
          if (subs.length && runData.goal === goalSel.value && runData.sub)
            subSel.value = runData.sub;
          valueField.value =
            goalSel.value + (subs.length ? '/' + subSel.value : '');
        }
        goalSel.value = runData.goal || 'unclassified';
        goalSel.addEventListener('change', paintMove);
        subSel.addEventListener('change', function () {
          valueField.value = goalSel.value + '/' + subSel.value;
        });
        paintMove();
      });
    }
    ...
    if (!isAuthor && !runData.imported) {
      if (!runData.videoOnly) {
        if (runData.reproduced.indexOf(myName) < 0)
          arm('f-repro', '/api/reproduce');
      }
      ...
      if (runData.openCase) {
        if (
          ...
          runData.openCase.voted.indexOf(myName) < 0
        ) {
          var voteForm = document.getElementById('f-vote');
          voteForm.hidden = false;
          anything = true;
          voteForm
            .querySelectorAll('button[data-reaffirm]')
            .forEach(function (b) {
      ...
      if (deleteForm)
        deleteForm.addEventListener(
          ...
          function (ev) {
            if (
              !window.confirm(
                'Delete this run outright? This cannot be undone, ' +
...
if (likeDataEl) {
  ...
  mePromise.then(function (d) {
    ...
    function paint(liked) {
      ...
      likeBtn.title = liked
        ? 'You like this run; press again to take it back'
        : 'Like this run';
    }
...
function adultOk() {
  try {
    return sessionStorage.getItem('tar-adult') === '1';
  } catch (e) {
    return false;
  }
}
...
function unblurAll() {
  document.querySelectorAll('.nsfwblur').forEach(function (i) {
    i.classList.remove('nsfwblur');
  });
  document.querySelectorAll('.nsfw18').forEach(function (b) {
    b.remove();
  });
}
...
function revealGate() {
  ...
  if (nsfwGate && realContent)
    nsfwGate.replaceWith(realContent.content.cloneNode(true));
}
...
function adultYes() {
  try {
    sessionStorage.setItem('tar-adult', '1');
  } catch (e) {}
  unblurAll();
  revealGate();
  liftAgeGate();
...
if (discussion && api) {
  ...
  var renderPosts = function (d) {
    if (!d.posts || !d.posts.length) {
      postsBox.innerHTML =
        '<p class="emptynote">No posts yet. Be the first to say ' +
        'something about this run.</p>';
      return;
    }
    postsBox.innerHTML = d.posts
      .map(function (p) {
        return (
          '<article class="dpost"><div class="dhead">' +
          (p.avatar
            ? '<img class="davatar" src="' +
              escapeHtml(p.avatar) +
              '" alt="" loading="lazy">'
            : '') +
          '<b>' +
          ...
          '</b>' +
          '<span class="actmeta">' +
          escapeHtml((p.date || '').replace('T', ' ')) +
          '</span>' +
          '<a class="actmeta" href="' +
          escapeHtml(d.url) +
          '/' +
          (p.number || 1) +
          '">#' +
  ...
  var loadDiscussion = function () {
    return fetch(api + '/api/discussion?topic=' + encodeURIComponent(topicId), {
      ...
      .catch(function () {
        postsBox.innerHTML =
          '<p class="emptynote">The discussion could not be loaded. ' +
          'Read it on <a href="' +
          escapeHtml(discussion.dataset.url) +
          '">the forum</a>.</p>';
      });
  };
  loadDiscussion();
  mePromise.then(function (me) {
    ...
    replyForm.addEventListener('submit', function (ev) {
      ...
      post('/api/discussion/reply', fd, btn).then(function (res) {
        btn.disabled = false;
        if (!res.ok) {
          note(msg, res.j.error || 'could not post', false);
          return;
        }
...
var TEXT_DOWNLOAD_RE =
  /\.(chimeraproject|json|txt|text|fm2|fm3|dsm|jrsr|mc2|ymv|bkm|3ct|tas|ctas|dft|htas|hltas|p2tas|srctas|qtas|mctas|inputs|itf|otts|ronr|sub|rec|csv|tsv|log|yaml|yml|xml|ini|cfg|conf|lua|py|sh|bash|bat|cmd|md|markdown|diff|patch)$/i;
