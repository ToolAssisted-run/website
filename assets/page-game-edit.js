...
if (gameEditEl) {
  ...
  mePromise.then(function (d) {
    ...
    if (d.unreachable) {
      gate.textContent =
        'The archivist is unreachable, so who you are cannot ' +
        'be checked right now. Reading works; editing will not.';
      return;
    }
    if (!d.loggedIn) {
      gate.innerHTML =
        'This page is for the experts covering this game. ' +
        '<a href="' +
        api +
    ...
    if (
      viewAsCoverage(gameEditData.experts, d.user.toLowerCase()).indexOf(
        d.user.toLowerCase()
      ) < 0 &&
    ...
    function cropCanvasBlob(source, sx, sy, sw, sh) {
      return new Promise(function (resolve, reject) {
        ...
        if (!ctx) {
          reject(new Error('thumbnail crop is not supported by this browser'));
          return;
        }
    ...
    function imageFrame() {
      ...
      var stageW = cropStage.clientWidth,
    ...
    function paintCrop() {
      if (!cropGeom) return;
      cropBox.style.left = cropGeom.x + 'px';
      cropBox.style.top = cropGeom.y + 'px';
      cropBox.style.width = cropGeom.w + 'px';
      cropBox.style.height = cropGeom.h + 'px';
    }
    function layoutCrop() {
      ...
      var w = maxW * scale,
        h = (w * 9) / 16;
      cropImage.style.left = frame.left + 'px';
      cropImage.style.top = frame.top + 'px';
      cropImage.style.width = frame.width + 'px';
      cropImage.style.height = frame.height + 'px';
      cropGeom = {
        frame: frame,
    ...
    function clampCrop() {
      ...
      cropGeom.x = Math.max(
        f.left,
        Math.min(cropGeom.x, f.left + f.width - cropGeom.w)
      );
      cropGeom.y = Math.max(
        f.top,
        Math.min(cropGeom.y, f.top + f.height - cropGeom.h)
      );
      paintCrop();
    }
    function openCrop() {
    ...
    function cropResult() {
      ...
      if (!f)
        return Promise.reject(new Error('choose an image before cropping'));
      return cropCanvasBlob(
        thumbSource,
    ...
    function autoCrop() {
      ...
      var sw =
        ratio >= 16 / 9
          ? (thumbSource.naturalHeight * 16) / 9
          : thumbSource.naturalWidth;
      var sh = (sw * 9) / 16;
      return cropCanvasBlob(
        thumbSource,
        (thumbSource.naturalWidth - sw) / 2,
        (thumbSource.naturalHeight - sh) / 2,
        sw,
        sh
      );
    }
    function prepareThumbnail() {
      ...
      if (!thumbSource)
        return Promise.reject(new Error('choose a thumbnail image'));
      return autoCrop().then(function (blob) {
        preparedThumb = blob;
        return blob;
      });
    }
    if (thumbInput) {
      thumbInput.addEventListener('change', function () {
        ...
        image.onerror = function () {
          thumbPreview.hidden = true;
          note(msg, 'The selected file is not a readable image.', false);
        };
      ...
      byId('ge-crop-size').addEventListener('input', function () {
        if (!cropGeom) return;
        var old = {
          x: cropGeom.x + cropGeom.w / 2,
          y: cropGeom.y + cropGeom.h / 2,
        };
        var maxW = Math.min(
          cropGeom.frame.width,
          (cropGeom.frame.height * 16) / 9
        );
        cropGeom.w = (maxW * Number(cropSize.value)) / 100;
        cropGeom.h = (cropGeom.w * 9) / 16;
        cropGeom.x = old.x - cropGeom.w / 2;
        cropGeom.y = old.y - cropGeom.h / 2;
        clampCrop();
      });
      ...
      cropStage.addEventListener('pointerdown', function (ev) {
        if (
          !cropGeom ||
          (ev.target !== cropBox && !cropBox.contains(ev.target))
        )
          return;
        cropDragging = true;
        cropDragX = ev.clientX;
        cropDragY = ev.clientY;
        cropBox.setPointerCapture(ev.pointerId);
      });
      cropStage.addEventListener('pointermove', function (ev) {
        if (!cropDragging || !cropGeom) return;
        cropGeom.x += ev.clientX - cropDragX;
        cropGeom.y += ev.clientY - cropDragY;
        cropDragX = ev.clientX;
        cropDragY = ev.clientY;
        clampCrop();
      });
    ...
    var base = {
      ...
      cats: (gameEditData.options || []).map(function (o) {
        return {
          ...
          subSelector: o.subSelector || 'buttons',
          subs: (o.subcategories || []).map(function (x) {
            return {
              key: x.key,
              label: x.label,
              rule: x.rule || '',
    ...
    selectorRadios.forEach(function (r) {
      r.checked = r.value === base.selector;
      r.addEventListener('change', refresh);
    });
    function selectorChoice() {
      var on = Array.prototype.filter.call(selectorRadios, function (r) {
        return r.checked;
      })[0];
    ...
    function gripIcon(size) {
      ...
      [
        ...
        var cEl = document.createElementNS(
    ...
    function orderArrows(list, item, render) {
      ...
      [
        ['up', -1, 'Move up (earlier)'],
        ['down', 1, 'Move down (later)'],
      ].forEach(function (spec) {
        var j = i + spec[1];
        if (j < 0 || j >= list.length) return;
        var b = el('button', 'orderbtn ' + spec[0]);
        b.type = 'button';
        b.title = spec[2];
    ...
    function card(c) {
      ...
      toggleBtn.setAttribute(
        'aria-label',
        (c._open ? 'Collapse ' : 'Expand ') + (c.label || c.key || 'category')
      );
      ...
      if (!c.runs) {
        ...
        del.addEventListener('click', function (ev) {
          ev.stopPropagation();
          if (c.isNew) {
            draft.cats.splice(draft.cats.indexOf(c), 1);
            renderCards();
            return;
          }
          c.deleted = !c.deleted;
      ...
      if (c.deleted) {
        body.appendChild(
          el('p', 'rules', 'Marked for deletion; Save removes it.')
        );
      ...
      if (!c.isNew) {
        ...
        var keyIn = field(
          'Key (lowercase-with-hyphens: the address rankings and links use; runs follow a rename)'
        );
        keyIn.value = c.newKey;
        keyIn.maxLength = 60;
        keyIn.pattern = '[a-z0-9]+(-[a-z0-9]+)*';
      ...
      var ruleIn = field('Rule (markdown)', 'textarea');
      ruleIn.value = c.rule;
      ruleIn.rows = 4;
      ruleIn.maxLength = 2000;
      ruleIn.addEventListener('input', function () {
        c.rule = ruleIn.value;
        refresh();
      });

      // Preview button logic for rules
      var rulePvBtn = el('button', 'btn quiet', 'Preview');
      rulePvBtn.type = 'button';
      rulePvBtn.style.marginTop = '8px';
      var rulePvBox = el('div', 'ge-rule-pv');
      rulePvBox.hidden = true;
      rulePvBox.style.marginTop = '8px';
      ...
      rulePvBtn.addEventListener('click', function () {
        if (rulePvBox.hidden) {
          ...
          post('/api/preview', fd, rulePvBtn)
            .then(function (res) {
              rulePvContent.innerHTML =
                res.ok && res.j.ok
                  ? res.j.html
                  : escapeHtml((res.j && res.j.error) || 'preview failed');
            })
            .catch(function () {
              rulePvContent.textContent =
      ...
      if (!c.isNew)
        base.cats.forEach(function (b) {
          if (b.key === c.key && !b.metricsNormalized) {
            b.metrics = metricsEd.value();
      ...
      ['buttons', 'dropdown'].forEach(function (v) {
        ...
        lab.appendChild(
          el(
            ...
            '',
            v === 'buttons'
              ? 'one button each (default)'
              : 'a dropdown (for long lists)'
          )
        );
      ...
      function subRow(sc) {
        ...
        sHandle.addEventListener('click', function (ev) {
          ev.stopPropagation();
        });
        ...
        sToggle.setAttribute(
          'aria-label',
          (sc._open ? 'Collapse ' : 'Expand ') +
            (sc.label || sc.key || 'subcategory')
        );
        ...
        var sLabelText = el(
          'span',
          'gesub-label-text',
          sc.label || sc.key || '(new subcategory)'
        );
        var sKeyDisplay = el(
          'code',
          'geslug-code',
          sc.key || '(assigned on save)'
        );
        ...
        var live = c.subs.filter(function (x) {
          return !x.deleted;
        });
        if (!sc.runs || live.length === 1) {
          ...
          sDel.addEventListener('click', function (ev) {
            ev.stopPropagation();
            if (sc.isNew) {
              c.subs.splice(c.subs.indexOf(sc), 1);
              renderSubs();
              return;
            }
            sc.deleted = !sc.deleted;
            renderSubs();
          });
        ...
        if (sc.deleted) {
          sBody.appendChild(
            el(
              'p',
              'rules',
              (sc.label || 'Subcategory') + ': marked for deletion'
            )
          );
        } else {
          ...
          r.maxLength = 2000;
          r.rows = 2;
          r.placeholder = 'rule fragment, markdown (optional)';
          rLab.appendChild(r);
          ...
          var subPvBtn = el('button', 'btn quiet', 'Preview');
          subPvBtn.type = 'button';
          var subPvBox = el('div', 'ge-subrule-pv');
          ...
          subPvBtn.addEventListener('click', function () {
            if (subPvBox.hidden) {
              ...
              fd.append('kind', 'rules');
              post('/api/preview', fd, subPvBtn)
                .then(function (res) {
                  subPvContent.innerHTML =
                    res.ok && res.j.ok
                      ? res.j.html
                      : escapeHtml((res.j && res.j.error) || 'preview failed');
                })
                .catch(function () {
                  subPvContent.textContent =
                    'The archivist is not reachable; the preview needs it.';
                });
            } else {
              subPvBox.hidden = true;
      ...
      var addLabel = el('input');
      addLabel.placeholder = 'new subcategory label, e.g. any%';
      addLabel.maxLength = 80;
      var addBtn = el('button', 'btn leave', '+ Add a subcategory');
      addBtn.type = 'button';
    ...
    function renderCards() {
      box.innerHTML = '';
      draft.cats.forEach(function (c) {
        box.appendChild(card(c));
      });
      refresh();
    }
    byId('ge-addcat').addEventListener('click', function () {
      var label = window.prompt(
        'Label of the new category (e.g. 100% completion):'
      );
    ...
    function plan() {
      ...
      var pv = props();
      ['released', 'unofficial', 'discord', 'website', 'rta', 'rules'].forEach(
        function (f) {
        ...
        .forEach(function (c) {
          ops.push({
            ...
            run: function () {
              ...
              fd.append('rule', c.rule);
              fd.append('metrics', c.metricsEd ? c.metricsEd.value() : '[]');
            ...
            .forEach(function (x) {
              ops.push({
                ...
                run: function () {
                  var fd = new FormData();
                  fd.append('game', gameEditData.game);
                  fd.append('parent', c.key);
                  fd.append('label', x.label);
                  fd.append('rule', x.rule);
                  fd.append('reason', whyIn.value.trim());
                  return post('/api/category/add', fd, saveBtn);
                },
                done: function (res) {
            ...
            .forEach(function (x) {
              ops.push({
                ...
                run: function () {
                  var fd = new FormData();
                  fd.append('game', gameEditData.game);
                  fd.append('option', c.key);
                  fd.append('sub', x.key);
                  fd.append('reason', whyIn.value.trim());
                  return post('/api/category/delete', fd, saveBtn);
                },
                done: function () {
                  c.subs.splice(c.subs.indexOf(x), 1);
                  if (b)
                    b.subs = b.subs.filter(function (z) {
                      return z.key !== x.key;
                    });
                },
              });
        ...
        .forEach(function (c) {
          ops.push({
            ...
            run: function () {
              ...
              draft.cats.splice(draft.cats.indexOf(c), 1);
              base.cats = base.cats.filter(function (z) {
                return z.key !== c.key;
              });
            },
          });
      ...
      var wantOrder = draft.cats
        .filter(function (c) {
          return !c.deleted;
        })
        .map(function (c) {
          return c.key || 'new:' + c.tmp;
        });
    ...
    if (rulesPvBtn && rulesPvBox && rulesPvContent) {
      rulesPvBtn.addEventListener('click', function () {
        if (rulesPvBox.hidden) {
          ...
          fd.append('kind', 'rules');
          post('/api/preview', fd, rulesPvBtn)
            .then(function (res) {
              rulesPvContent.innerHTML =
                res.ok && res.j.ok
                  ? res.j.html
                  : escapeHtml((res.j && res.j.error) || 'preview failed');
            })
            .catch(function () {
              rulesPvContent.textContent =
                'The archivist is not reachable; the preview needs it.';
            });
        } else {
    ...
    saveBtn.addEventListener('click', function () {
      ...
      if (whyIn.value.trim().length < 8) {
        whyIn.focus();
        note(msg, 'Say why, publicly: at least 8 characters.', false);
        return;
      }
      msg.hidden = true;
      var n = 0,
      ...
      (function step() {
        ...
        op.run().then(function (res) {
          if (res.ok && res.j.ok) {
            op.done(res);
            n++;
            lastSerial = res.j.serial || lastSerial;
            step();
          } else {
            setLastBtn(saveBtn);
            note(
              ...
              op.what +
                ': ' +
                (res.j.error || 'something went wrong') +
                (n
                  ? ' (' +
                    n +
                    ' change' +
