...
for _e in edit_events:
    ...
    if _e.get('kind') == 'category':
        _k = _e.get('key', '')
        if ':' in _k:
            category_edits_by_game.setdefault(_k.split(':', 1)[0], []).append(_e)
...
if (ARCHIVE / 'groups.json').exists():
    ...
    groups_by_key = {gr['key']: gr for gr in all_groups}
else:
    groups_by_key = {}
...
for r in withdrawn_runs:
    withdrawn_by_game.setdefault(r['_game']['key'], []).append(r)

runs = [r for r in runs if not r.get('withdrawn')]
runs_by_id = {r['id']: r for r in runs + withdrawn_runs}
...
REMOVAL_CHIPS = {
    'open': ('pendchip', 'Open'),
    'granted': ('verchip', 'Granted'),
    'declined': ('', 'Declined'),
}

def diff_games(before_s, after_s, reason=''):
    """Compute (removed, added) game items between two revisions of a group's games.

    Uses structured reason summary from archivist if present (which records
    exact additions, drops, and cross-group moves without string truncation);
    otherwise falls back to parsing comma-separated lists, filtering out any
    truncated fragments or non-game tokens.
    """
    added = []
    removed = []

    reason = str(reason or '')
    if 'Changed from the group form:' in reason:
        # Structured reasons generated by archivist
        for m in re.finditer(r'([a-z0-9_-]+/[a-z0-9_-]+)\s+moved in(?:\s+from\s+([a-z0-9_-]+))?', reason):
            k = m.group(1)
            if not any(x['key'] == k for x in added):
                added.append({'key': k, 'from_group': m.group(2)})
        for m in re.finditer(r'([a-z0-9_-]+/[a-z0-9_-]+)\s+moved out(?:\s+to\s+([a-z0-9_-]+))?', reason):
            k = m.group(1)
            if not any(x['key'] == k for x in removed):
                removed.append({'key': k, 'to_group': m.group(2)})
        for m in re.finditer(r'(?:^|[\s,])\+([a-z0-9_-]+/[a-z0-9_-]+)', reason):
            k = m.group(1)
            if not any(x['key'] == k for x in added):
                added.append({'key': k, 'from_group': None})
        for m in re.finditer(r'(?:^|[\s,])-([a-z0-9_-]+/[a-z0-9_-]+)', reason):
            k = m.group(1)
            if not any(x['key'] == k for x in removed):
                removed.append({'key': k, 'to_group': None})
    ...
    if not added and not removed:
        ...
        before = parse_keys(before_s)
        after = parse_keys(after_s)
        before_set = set(before)
        after_set = set(after)
        for x in before:
            if x not in after_set and not any(r['key'] == x for r in removed):
                removed.append({'key': x, 'to_group': None})
        for x in after:
            if x not in before_set and not any(a['key'] == x for a in added):
                added.append({'key': x, 'from_group': None})
