...
def file_removal_request(holder, kind, name, expert, reason):
    """Append an open request. Shared by games and group: the shape is the
    same and so is the rule that only one can be open at a time."""
    reqs = holder.setdefault('removalRequests', [])
    if any(r['status'] == 'open' for r in reqs):
...
@app.post('/api/game/request-removal')
def game_request_removal():
    """Ask for a game to be taken out of the archive's index.

    An expert never deletes: they ask, in public, and somebody wider decides.
    A game holds other people's movies, and one expert's view that it should
    not exist is a proposal, not a fact.
    """
    f = request.form
    dry = f.get('dry_run') in ('1', 'true', 'yes')
    refresh_archive()
    with lock:
        ...
        reason = (f.get('reason') or '').strip()
        if not (8 <= len(reason) <= 500):
            return fail('say why, in a sentence somebody can answer')
        ...
        if not expert_covers(expert, game_key):
            return fail(f'{expert!r} is not an expert covering {game_key}', 403)
        game = json.loads(gfile.read_text())
        entry, err2 = file_removal_request(game, 'game', game_key, expert, reason)
        if err2:
            return err2
        if dry:
            return jsonify({'ok': True, 'dry_run': True, 'would_request': entry})
        checkout_branch()
        game = json.loads(gfile.read_text())
        entry, err2 = file_removal_request(game, 'game', game_key, expert, reason)
        if err2:
        ...
        gfile.write_text(json.dumps(game, indent=1))
        ensure_member(expert)
        commit_push(f'Removal requested for {game_key}: by {expert}\n\n'
                    f'Reason: {reason}\nVia: archivist')
    return jsonify({'ok': True, 'game': game_key, 'request': entry,
                    'note': 'Filed. A site-wide expert decides it, and the request '
                            'is public in the site log meanwhile.'})

@app.post('/api/group/request-removal')
def group_request_removal():
    """Ask for a group to be dissolved. Same rule: ask, do not delete."""
    f = request.form
    dry = f.get('dry_run') in ('1', 'true', 'yes')
    refresh_archive()
    with lock:
        ...
        entry, err2 = file_removal_request(gr, 'group', gr['title'], expert, reason)
        if err2:
            return err2
        if dry:
            return jsonify({'ok': True, 'dry_run': True, 'would_request': entry})
        ...
        save_groups(doc)
        ensure_member(expert)
        commit_push(f'Removal requested for group {key}: by {expert}\n\n'
                    f'Reason: {reason}\nVia: archivist')
    return jsonify({'ok': True, 'group': key, 'request': entry,
                    'note': 'Filed. A site-wide expert decides it.'})

@app.post('/api/removal/decide')
def removal_decide():
    """A site-wide expert answers a removal request.

    Granting takes the thing out of the index: a game keeps its page and its
    runs, marked removed, because the movies inside it are somebody else's
    work and were never what was in question; a group is dissolved and its
    games are ungrouped. Declining closes the request with a reason. Either
    way the request, the answer and both names stay in the archive.
    """
    f = request.form
    dry = f.get('dry_run') in ('1', 'true', 'yes')
    refresh_archive()
    with lock:
        ...
        if err_:
            return err_
        if not is_site_expert(expert):
            return fail('only a site-wide expert decides a removal request', 403)
        kind = (f.get('kind') or '').strip()
        key = (f.get('target') or '').strip()
        action = (f.get('action') or '').strip()
        note_ = (f.get('note') or '').strip()
        if kind not in ('game', 'group'):
            return fail('kind must be game or group')
        if action not in ('granted', 'declined'):
            return fail('action must be granted or declined')
        if action == 'declined' and not (8 <= len(note_) <= 500):
            return fail('declining needs a reason the requester can read')
        gfile = None
        if kind == 'game':
            if not re.fullmatch(r'[a-z0-9-]+/[a-z0-9-]+', key):
                return fail('target must be system/slug for a game')
            gfile = ARCHIVE / 'games' / key / 'game.json'
            if not gfile.exists():
                return fail(f'unknown game {key}', 404)
            holder = json.loads(gfile.read_text())
        else:
            doc = load_groups()
            holder = next((g for g in doc['groups'] if g['key'] == key.lower()), None)
            if not holder:
                return fail(f'no group with the key {key!r}', 404)
        req = next((r for r in holder.get('removalRequests', [])
                    if r['status'] == 'open'), None)
        if not req:
            return fail(f'no removal request is open for {key}', 404)
        today_ = time.strftime('%Y-%m-%d', time.gmtime())
        if dry:
            return jsonify({'ok': True, 'dry_run': True, 'request': req,
                            'would': action})
        checkout_branch()
        if kind == 'game':
            holder = json.loads(gfile.read_text())
        ...
        if action == 'granted':
            ...
            if kind == 'group':
                ...
                holder['games'] = []
        if kind == 'game':
            gfile.write_text(json.dumps(holder, indent=1))
        else:
            save_groups(doc)
        ensure_member(expert)
        commit_push(f'Removal {action} for {kind} {key}: by {expert}\n\n'
                    f'Requested by: {req["by"]}\nReason: {req["reason"]}\n'
                    + (f'Note: {note_}\n' if note_ else '')
                    + f'Via: archivist')
...
@app.post('/api/note')
def role_note():
    """Shared role notes: one collaborative box each for reproducers,
    verifiers, and the game's (or game group's) experts — always visible on
    the run page, with an attribution list of everyone who edited it."""
    f = request.form
    dry = f.get('dry_run') in ('1', 'true', 'yes')
    refresh_archive()
    with lock:
        ...
        text = (f.get('notes') or '').strip()
        if len(text) > 4000:
            return fail('notes exceed 4000 characters')
        if role == 'reproducer':
            if user.lower() not in {a['user'].lower() for a in r.get('reproductions', [])}:
                return fail('only members who reproduced this run may edit the '
                            'reproducer notes')
        elif role == 'verifier':
            if user.lower() not in {a['user'].lower() for a in r.get('verifications', [])}:
                return fail('only members who verified this run may edit the '
                            'verifier notes')
        elif role == 'expert':
            # deliberately narrower than other expert acts: the game's or game
            # group's experts only — site-wide scope does not qualify
            sys_ = r['game'].split('/')[0]
            reach = scopes_over(r['game']) - {'site'}
            if not any(e['user'].lower() == user.lower() and e['scope'] in reach
                       for e in load_experts()):
                return fail(f'only experts of {sys_}, {r["game"]} or its game '
                            f'group may edit the expert notes', 403)
        else:
            return fail('role must be reproducer, verifier, or expert')
        rn = r.setdefault('roleNotes', {})
        entry = rn.get(role) or {'text': '', 'editors': []}
        entry['text'] = text
        today = time.strftime('%Y-%m-%d', time.gmtime())
        mine = next((e for e in entry['editors'] if e['user'].lower() == user.lower()), None)
        if mine:
            mine['date'] = today
        else:
        ...
        (rdir / 'run.json').write_text(json.dumps(
            {k: v for k, v in r.items() if not k.startswith('_')}, indent=1))
        ensure_member(user)
        commit_push(f'Note on {run_id}: {role} notes by {user}\n\nVia: archivist')
    return jsonify({'ok': True, 'run': run_id, 'role': role,
                    'editors': [e['user'] for e in entry['editors']]})
